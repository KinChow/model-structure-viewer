...
test("edgeStrokeWidth 弱化顺序推断边，声明边按张量规模取宽", () => {
  ...
  const declaredSmall = edgeStrokeWidth({ kind: "dataflow", evidence: "declared" }, { node: { output_shape: [-1, -1, 8] } });
  const declaredLarge = edgeStrokeWidth({ kind: "dataflow", evidence: "declared" }, { node: { output_shape: [1, 4096, 4096] } });
