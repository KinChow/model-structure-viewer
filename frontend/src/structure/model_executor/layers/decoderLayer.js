...
function decoderLayerEdges({ layerKind, isMhc, isQwen4Exp, isLastLayer, hasPle, hasHyper, hasAttnRes }) {
  const ffn = ffnRef(layerKind);
  if (isMhc) {
    return [
      ["mhc_attn_pre", "self_attn"],
  ...
  if (isQwen4Exp) {
    ...
    edges.push(
      ["attn_hyper_connection", "self_attn"],
      ["self_attn", "attn_residual_add"],
...
export function decoderLayerModule(id, normalized, { layerKind, attentionKind, layerIndex = 0 }) {
  ...
  const ffn = layerKind === "moe" ? moeModule(`${id}.moe`, normalized, { layerIndex }) : mlpModule(`${id}.mlp`, normalized);
