    ...
    <main className={`detail-page theme-${theme}`}>
      ...
      <section className="detail-layout">
        <div className="detail-main">
          ...
          <div className="detail-cost-toggle"><button type="button" onClick={() => setCostOpen((value) => !value)} aria-expanded={costOpen} aria-controls="detail-cost-panel"><span>{t.cost}</span><span className="detail-cost-summary">{activeMode === "pd" ? "PD" : "Centralized"} · {activeMachineName} · {activePhase} · {activeNodeCount}×{activeGpusPerNode} GPU</span>{fitLabel && <span className={`detail-fit-status ${costFitStatus.fit ? "fit" : costFitStatus.known ? "no-fit" : "unknown"}`}>{fitLabel}</span>}<span>{costOpen ? "−" : "+"}</span></button></div>
          <div id="detail-cost-panel" className={`detail-cost-panel${costOpen ? "" : " is-collapsed"}`} aria-hidden={!costOpen}><CostSummary structure={structure} chips={chips} onAddChip={onAddChip} language={language} onFitStatusChange={setCostFitStatus} lenses={activeLenses} onLensesChange={setActiveLenses} phase={activePhase} onPhaseChange={changeActivePhase} mode={activeMode} onModeChange={changeActiveMode} plans={activePlans} onPlansChange={setActivePlans} nodes={activeNodes} onNodesChange={setActiveNodes} gpusPerNode={activeGpusPerNode} onGpusPerNodeChange={setActiveGpusPerNode} machineId={activeMachineId} onMachineIdChange={setActiveMachineId} loads={activeLoads} onLoadsChange={setActiveLoads} /></div>
