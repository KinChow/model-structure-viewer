...
| **N2-4 权重归属声明与分片响应矩阵** | W-A 已落地（2026-09-09，[details/sharding_matrix.md](details/sharding_matrix.md)）：fused_moe_mlp 独立 id + 层 1 weightMatrices 声明（统一助手 weightMatrixDecl，专家 ep 组 / attention·dense linear tp 组）+ 锚 1 全目录 5233 声明叶容差 0；W-B 消费者接线（moe_tp/moe_ep + sharding.js + 三消费者）、W-C 覆盖推进（25 量化模型核实 + 销案）待做 | 量化 MoE 容量 2× 偏高（25 模型实证）+ EP/TP 投影派生路径塌缩 + 排除桶错误——三层设计（叶声明/计划轴/内存类响应）三消费者接线 | 四验收锚（单源/EP 自洽/回退不变/golden 纪律） |
