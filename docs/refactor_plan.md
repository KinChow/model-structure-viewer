...
| **N2-4 权重归属声明与分片响应矩阵** | W-A/W-B 已落地（2026-09-09，[details/sharding_matrix.md](details/sharding_matrix.md)）：W-A = fused_moe_mlp 独立 id + 层 1 weightMatrices 声明 + 锚 1 全目录 5233 声明叶容差 0；W-B = 计划轴 moe_tp/moe_ep + sharding.js 组合语义纯函数 + 三消费者接线（声明优先、规则表回退）+ 锚 2 三方一致 + 量化 MoE 容量回落（M2.7 2.52e11 / V4-Pro 1.69e12，此前 ≈2× 偏高）；W-C 覆盖推进（25 量化模型逐模型核实 + 家族差异表 + 销案）待做 | 量化 MoE 容量 2× 偏高（25 模型实证）+ EP/TP 投影派生路径塌缩 + 排除桶错误——三层设计（叶声明/计划轴/内存类响应）三消费者接线 | 四验收锚（单源/EP 自洽/回退不变/golden 纪律） |
