...
export function rmsNormModule(id, name = "RMSNorm", normalized = null, role = undefined) {
  ...
  const gemmaStyle = deriveBuildPlan(normalized?.raw ?? normalized).normMode === "gemma_rmsnorm";
  const formulaId = gemmaStyle ? "gemma_rmsnorm" : "rmsnorm";
  const className = gemmaStyle
    ? "GemmaRMSNorm"
    : hfNamedClass(normalized, "rmsNormStem", "RMSNorm", "RMSNorm");
