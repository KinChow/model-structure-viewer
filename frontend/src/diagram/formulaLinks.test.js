...
test("收集公式节点路径并保留解释", () => {
  const links = collectFormulaLinks({ attributes: {}, children: [{ attributes: { operator_id: "linear", explanation: "矩阵乘" }, children: [] }] });
...
test("Graph IR formula links use stable node paths", () => {
  const links = collectFormulaLinks({
    nodes: [{ id: "root.2.0", attributes: { operator_id: "attention", explanation: "QK" } }],
  });
