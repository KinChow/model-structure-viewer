...
test("unsupported builds an explicit empty network instead of a guessed structure", () => {
  const network = buildNetwork(
    { canonicalArchitecture: "unsupported", architecture: "MysteryForCausalLM" },
    { modelType: "mystery" },
  );
  assert.equal(network.kind, "network");
  assert.equal(network.canonicalArchitecture, "unsupported");
...
test("unsupported diagnostic enumerates supported architectures", () => {
  ...
  assert.equal(resolved.canonicalArchitecture, "unsupported");
