    ...
    <main className={`detail-page theme-${theme}`}>
      ...
      <section className="detail-layout">
        <div className="detail-main">
          ...
          {costOpen && <div className="detail-cost-panel"><CostSummary structure={structure} chips={chips} onAddChip={onAddChip} language={language} lenses={activeLenses} onLensesChange={setActiveLenses} phase={activePhase} onPhaseChange={setActivePhase} mode={activeMode} onModeChange={setActiveMode} plans={activePlans} onPlansChange={setActivePlans} nodes={activeNodes} onNodesChange={setActiveNodes} gpusPerNode={activeGpusPerNode} onGpusPerNodeChange={setActiveGpusPerNode} /></div>}
          <ArchitectureTab structure={structure} zoom={zoom} onZoomChange={onZoomChange} fitNonce={fitNonce} onFit={onFit} selectedPath={selectedPath} matchedPaths={matchedPaths} expandedGroups={expandedGroups} searchActive={Boolean(searchTerm.trim())} hitCount={matchedPaths.size} onSelectNode={onSelectNode} onToggleGroup={onToggleLayerPath} onExpandAllGroups={onExpandAllLayers} onCollapseAllGroups={onCollapseAllLayers} chips={chips} onAddChip={onAddChip} language={language} activeLenses={activeLenses} activePhase={activePhase} activeMode={activeMode} activePlans={activePlans} onPlanChange={setActivePlans} activeNodes={activeNodes} gpusPerNode={activeGpusPerNode} onNodeLensChange={setNodeLens} compactControls />
        ...
        <div className="detail-inspector-slot">{selectedData ? <NodeDetailPanel node={selectedData} path={selectedPath} breadcrumbs={breadcrumbs} totalParameters={structure?.summary?.parameters_total} costLens={nodeLens?.[selectedPath]} language={language} onSelectPath={onSelectNode} onClose={onCloseNode} /> : <div className="model-inspector-summary"><span className="inspector-kicker">MODEL SUMMARY</span><h2>{structure?.summary?.model_family || structure?.summary?.model_type || "Model"}</h2><p>{language === "en" ? "Select a structure node to inspect parameters, shapes, weights, and cost." : "选择结构节点查看参数、Shape、权重和成本。"}</p><div className="inspector-rule" /></div>}</div>
      </section>
    </main>
  );
}
