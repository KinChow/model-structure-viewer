# DeepSeek-V4.1-Flash config + safetensors index 逐层实证（2026-09-17）
...
## 结论：解决「compressor/indexer 层位」并修前端保真差
...
dtype/shape，与 builtin `header-truth.json` 对账（逐张量产物不入库，可由 range-read 探针重生）：
