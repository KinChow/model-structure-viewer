...
function ConfigSection({ id, title, summary, open, onToggle, children }) {
  return (
    <section className={`cost-config-section${open ? " is-open" : ""}`} data-section={id}>
      <button type="button" className="cost-config-section-toggle" aria-expanded={open} onClick={onToggle}>
        ...
        {!open && summary ? <span className="cost-config-section-summary">{summary}</span> : null}
...
function TopologyGrid({ nodes, gpusPerNode, tp, required, total, english }) {
  if (!(nodes > 0) || !(gpusPerNode > 0)) return null;
  const over = required > total;
  if (total > 64 || nodes > 8 || gpusPerNode > 16) {
    return <div className={`cost-topo compact${over ? " over" : ""}`}><span>{english ? "Topology" : "拓扑"}: {nodes} × {gpusPerNode} GPU = {total} · {english ? `using ${required}` : `用 ${required} 卡`}</span>{over && <span className="topo-warn-inline">{english ? "needs more GPUs" : "卡数不足"}</span>}</div>;
  }
  const rows = [];
  let idx = 0;
  for (let n = 0; n < nodes; n += 1) {
    ...
    for (let g = 0; g < gpusPerNode; g += 1) {
      const used = idx < required;
      const grp = used ? Math.floor(idx / Math.max(tp, 1)) % 2 : 0;
      cells.push(<span key={g} className={`topo-gpu${used ? ` used tp${grp}` : ""}`}>{used ? idx : "·"}</span>);
      idx += 1;
    }
    rows.push(<div key={n} className="topo-node"><span className="topo-nlab">N{n}</span><span className="topo-gpus">{cells}</span></div>);
  }
  return <div className={`cost-topo${over ? " over" : ""}`}><div className="topo-cap"><span>{english ? "Topology (node × GPU)" : "拓扑映射（节点 × 卡）"}</span><span>{english ? `using ${required} / ${total}` : `用 ${required} / ${total} 卡`}</span></div><div className="topo-nodes">{rows}</div>{over && <div className="topo-warn">{english ? "Plan needs more GPUs than the topology provides — raise nodes/GPU-per-node or lower TP×PP×DP." : "并行所需卡数超过当前拓扑：增加节点/单节点 GPU，或降低 TP×PP×DP。"}</div>}</div>;
}
...
export default function CostSummary({ structure, chips = PUBLIC_CHIPS, onAddChip, language = "zh", onFitStatusChange, lenses: controlledLenses, onLensesChange, phase: controlledPhase, onPhaseChange, mode: controlledMode, onModeChange, plans: controlledPlans, onPlansChange, nodes: controlledNodes, onNodesChange, gpusPerNode: controlledGpusPerNode, onGpusPerNodeChange, machineId: controlledMachineId, onMachineIdChange, loads: controlledLoads, onLoadsChange, comparisonMode = "off", onComparisonModeChange, compareChipId = "", onCompareChipIdChange, comparePlan = DEFAULT_COMPARE_PLAN, onComparePlanChange, efficiency = DEFAULT_EFFICIENCY, onEfficiencyChange, frameworkProfile = "neutral", deploymentRecommendation, deploymentManual = false, onResetDeployment }) {
  ...
  // N4-第4项：跨节点带宽用户可调覆盖（GB/s，仅在 interNode 开启时生效）。默认 0 = 不覆盖，
  // 公开卡缺 inter_node 规格时保持 unknown。
  const [interNodeBandwidth, setInterNodeBandwidth] = useState(0);
  // N4-α（opt-in）：每次集合通信的固定延迟（µs）。默认 0 → comm 仍是纯带宽下界；
  // >0 时按 Hockney T=α·次数 + n/β 计入，comm 变"含固定开销的估计"（UI 标注）。
  const [commLatencyUs, setCommLatencyUs] = useState(0);
  ...
  // 用户可调跨节点带宽：UI 取 GB/s，内部换算 bytes/s；仅在 interNode 开启且 > 0 时生效。
  const interNodeOverrideBytesPerSecond = interNode && interNodeBandwidth > 0 ? interNodeBandwidth * 1e9 : undefined;
  // µs → s；默认 0 时传 undefined（roofline 视为无 α，逐位等于旧下界口径）。
  const commLatencySeconds = commLatencyUs > 0 ? commLatencyUs * 1e-6 : undefined;
  const roofline = useMemo(() => cost && machine ? classifyRoofline({
    ...
    actions: cost.actions && {
      ...
      commOps: communication?.totalOps || 0,
    },
  }, machine, { dtype: "bf16", efficiency, interNode, interNodeBandwidth: interNodeOverrideBytesPerSecond, commLatencySeconds }) : null, [cost, machine, communication, efficiency, interNode, interNodeOverrideBytesPerSecond, commLatencySeconds]);
  ...
  const stageRoofline = useMemo(() => {
    ...
    const opts = { dtype: "bf16", efficiency, interNode, interNodeBandwidth: interNodeOverrideBytesPerSecond, commLatencySeconds };
    ...
    const commOnly = Object.keys(commByGroup).filter((group) => !seen.has(group)).map((group) => ({
      group,
      actions: { matrix: 0, vector: 0, sfu: 0, bytes: { weights: 0, actIn: 0, actOut: 0, kvRead: 0, indexRead: 0 }, computeDtypes: {}, commBytes: commByGroup[group].bytes, commOps: commByGroup[group].ops },
    }));
  ...
  useEffect(() => {
    ...
    onFitStatusChange?.({ fit: planFitsTopology && planFitsMemory === true, known: planFitsMemory != null, status: planStatus, phaseFits: mode === "pd" ? { prefill: pdFit?.prefill?.fit, decode: pdFit?.decode?.fit } : null, totalVram: cost.memory.totalBytes, capacity: machine.memory_bytes, bound: roofline?.bound ?? null, boundLabel: summary?.boundLabel ?? null, boundCategoryLabel: summary?.boundCategoryLabel ?? null, requiredGpus, totalGpus });
  }, [cost, machine, mode, onFitStatusChange, pdFit, planFitsTopology, planFitsMemory, planStatus, roofline, summary, requiredGpus, totalGpus]);
  ...
  const vramScale = Math.max(cost.memory.totalBytes || 0, available || 0) * 1.02 || 1;
  const vpct = (b) => `${Math.max(0, ((b || 0) / vramScale) * 100)}%`;
  const rTimes = roofline?.times || {};
  const rCompute = Number.isFinite(rTimes.matrix) ? rTimes.matrix : null;
  const rMem = Number.isFinite(rTimes.memory) ? rTimes.memory : null;
  const rComm = Number.isFinite(rTimes.comm) ? rTimes.comm : null;
  const rMax = Math.max(rCompute || 0, rMem || 0, rComm || 0) || 1;
  const rHi = (key) => (key === "compute" ? ["matrix", "vector", "sfu"].includes(roofline?.bound) : roofline?.bound === key);
  ...
  return <section className="cost-summary cost-summary-modern" aria-label={text.estimate} data-framework={accounting.framework}>
    ...
    <div className="cost-lens-row"><span>Cost Lens</span>{[["none", "None", "", ""], ["compute", "Compute", "FLOPs · roofline compute-bound", "算力 FLOPs · roofline 计算瓶颈"], ["memory", "Memory traffic", "Bytes moved · roofline memory-bound", "访存流量（搬运字节）· roofline 访存瓶颈"], ["vram", "VRAM", "Memory footprint · includes weights + KV cache + state", "显存占用 · 含权重 + KV 缓存 + 状态"]].map(([id, label, tEn, tZh]) => <button type="button" key={id} title={id === "none" ? undefined : (english ? tEn : tZh)} className={(id === "none" ? lenses.size === 0 : lenses.has(id)) ? "active" : ""} aria-pressed={id === "none" ? lenses.size === 0 : lenses.has(id)} onClick={() => toggleLens(id)}>{label}</button>)}</div>
    ...
    {expanded && <div className="cost-config-modern">
      <ConfigSection id="machine" open={openConfigSections.machine} onToggle={() => toggleConfigSection("machine")} summary={`${machine?.name || "GPU"} · ${gpusPerNode} GPU/node`} title={text.machine}><MachineSection text={text} machine={machine} changeMachine={changeMachine} chips={chips} language={language} onAddChip={onAddChip} mode={mode} nodes={nodes} updateNodes={updateNodes} gpusPerNode={gpusPerNode} updateGpusPerNode={updateGpusPerNode} /></ConfigSection>
      <ConfigSection id="mode" open={openConfigSections.mode} onToggle={() => toggleConfigSection("mode")} summary={`${mode === "pd" ? "PD" : text.centralized} · seq ${load.sequence} · batch ${load.batch}`} title={text.mode}><ModeSection mode={mode} changeMode={changeMode} text={text} phase={phase} changePhase={changePhase} load={load} updateLoad={updateLoad} language={language} structure={structure} nodes={nodes} updateNodes={updateNodes} /></ConfigSection>
      <ConfigSection id="parallelism" open={openConfigSections.parallelism} onToggle={() => toggleConfigSection("parallelism")} summary={`TP${plans[phase].tp} · PP${plans[phase].pp ?? 1} · EP${plans[phase].ep} · DP${plans[phase].dp ?? 1}`} title={`${t(language, "cost.parallelism")}${mode === "pd" ? ` · ${phase}` : ""}`}>{deploymentRecommendation && <div className="cost-default-deployment" data-mode={deploymentManual ? "manual" : "auto"} data-cards={deploymentRecommendation.cards} data-fit={String(deploymentRecommendation.fit)}>
      ...
      </div>}<PlanFields plan={plans[phase]} english={english} onChange={(next) => updatePlan({ ...plans, [phase]: next })} /><TopologyGrid nodes={currentNodes} gpusPerNode={gpusPerNode} tp={plan.tp} required={requiredGpus} total={totalGpus} english={english} />{frameworkProfile === "sglang" && config.sharedExperts > 0 && <label className="cost-check"><input type="checkbox" checked={(plans[phase].enforceSharedExpertsFusion ?? plans[phase].enforce_shared_experts_fusion ?? false) === true} onChange={(event) => updatePlan({ ...plans, [phase]: { ...plans[phase], enforceSharedExpertsFusion: event.target.checked } })} />{english ? "Shared-expert fusion (opt-in)" : "共享专家融合（显式启用）"}</label>}</ConfigSection>
      <ConfigSection id="analysis" open={openConfigSections.analysis} onToggle={() => toggleConfigSection("analysis")} summary={`${comparisonMode === "off" ? text.off : comparisonMode === "chip" ? text.chipCompare : text.planCompare} · ηF ${efficiency.flops}`} title={text.analysis}><AnalysisSection text={text} comparisonMode={comparisonMode} onComparisonModeChange={onComparisonModeChange} compareChipId={compareChipId} onCompareChipIdChange={onCompareChipIdChange} chips={chips} comparePlan={comparePlan} onComparePlanChange={onComparePlanChange} efficiency={efficiency} onEfficiencyChange={onEfficiencyChange} etaNote={etaNote} /></ConfigSection>
      <ConfigSection id="assumptions" open={openConfigSections.assumptions} onToggle={() => toggleConfigSection("assumptions")} summary={`KV ${kvElementBytes} B/elem${interNode ? " · inter-node" : ""}`} title={t(language, "cost.assumptions")}><AssumptionsSection language={language} kvElementBytes={kvElementBytes} setKvElementBytes={setKvElementBytes} interNode={interNode} setInterNode={setInterNode} interNodeBandwidth={interNodeBandwidth} setInterNodeBandwidth={setInterNodeBandwidth} weightMode={weightMode} setWeightMode={setWeightMode} frameworkProfile={frameworkProfile} speculativeDraftTokens={speculativeDraftTokens} setSpeculativeDraftTokens={setSpeculativeDraftTokens} speculativeMaxRequests={speculativeMaxRequests} setSpeculativeMaxRequests={setSpeculativeMaxRequests} commLatencyUs={commLatencyUs} setCommLatencyUs={setCommLatencyUs} /></ConfigSection>
    </div>}
    ...
    <div className="cost-vram-bar"><div className={`cvb-track${(cost.memory.totalBytes || 0) > available && available > 0 ? " over" : ""}`}><span className="cvb-seg cvb-weights" style={{ width: vpct(cost.memory.weightBytes) }} /><span className="cvb-seg cvb-kv" style={{ width: vpct(cost.memory.kvBytes) }} /><span className="cvb-seg cvb-state" style={{ width: vpct(cost.memory.stateBytes) }} /><span className="cvb-seg cvb-buffers" style={{ width: vpct(cost.memory.bufferBytes || 0) }} />{available > 0 && <span className="cvb-cap" style={{ left: `${Math.min(100, (available / vramScale) * 100)}%` }} />}</div><div className="cvb-legend"><span><i className="cvb-dot cvb-weights" />{t(language, "cost.weights")} {formatBytes(cost.memory.weightBytes)}</span><span><i className="cvb-dot cvb-kv" />{t(language, "cost.kv")} {formatBytes(cost.memory.kvBytes)}</span><span><i className="cvb-dot cvb-buffers" />{english ? "Runtime/act" : "运行时/激活"} {formatBytes((cost.memory.bufferBytes || 0) + (cost.memory.stateBytes || 0))}</span>{available > 0 && <span className="cvb-cap-legend">▏ {english ? "Card" : "单卡容量"} {formatBytes(available)}</span>}</div></div>
