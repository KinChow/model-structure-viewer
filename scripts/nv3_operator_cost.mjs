// nv3_operator_cost.mjs —— NV-3 深化：dump Qwen3-0.6B 逐算子动作向量（前端 cost lens 真值）。
// 复用生产链路：buildStructureFromConfig → computeNodeCosts(graph, normalized, {batch,sequence,phase})
//   → aggregateNodeCosts（与 diagram/lens.js:28、cost/aggregate.js:67 同调用口径）。
// 产物 frontend_ops.json 供 reconcile.py 逐通道对 FlopCounterMode / ncu 真值。
...
const configPath = argVal("--config", "/ssd2/models/Qwen/Qwen3-0.6B/config.json");
const outDir = path.resolve(argVal("--out", path.join(repoRoot, "docs/details/nv_evidence/nv3/operator_cost")));
