...
async function smoke(cdp) {
  ...
  const graph = await page(cdp, `return {svg:Boolean(document.querySelector('svg[aria-label="Model architecture diagram"]')), nodes:document.querySelectorAll('g[data-node-path]').length, formulas:document.querySelectorAll('.formula-strip button').length, layersButton:Array.from(document.querySelectorAll('button')).some((b)=>b.textContent.trim()==='Layers')};`);
  const analysis = await clickText(cdp, ".toolbar-actions > button", "分析配置") || await clickText(cdp, ".toolbar-actions > button", "Analysis");
  const compare = await page(cdp, `const button=Array.from(document.querySelectorAll('.lens-mode-switch button')).find((b)=>b.textContent.trim()==='方案'); button?.click(); await new Promise((r)=>setTimeout(r,0)); return {analysis:${analysis},panes:document.querySelectorAll('.diagram-compare .diagram-frame').length,compareTp:document.body.innerText.includes('对比 TP')};`);
  const formula = await page(cdp, `const button=document.querySelector('.formula-strip button[data-node-path]'); button?.click(); await new Promise((r)=>setTimeout(r,0)); return {path:button?.dataset.nodePath||'',inspector:Boolean(document.querySelector('.detail-panel'))};`);
  await navigate(cdp);
  await openBuiltin(cdp);
