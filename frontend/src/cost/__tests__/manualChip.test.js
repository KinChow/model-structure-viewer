...
test("手工条目带 field_sources：已填字段逐项标注 user-input（旁路 C）", () => {
  const chip = createManualChip({ name: "My Chip", memoryGb: 64, memoryBandwidthTb: 2, bf16Tflops: 300, smTflops: 20, sfuTops: 5, interconnectGb: 100 });
  assert.deepEqual(chip.field_sources, {
    memory_bytes: "user-input",
    memory_bandwidth: "user-input",
    peak_flops: "user-input",
    vector_flops: "user-input",
    sfu_ops: "user-input",
  ...
  assert.equal(chip.vector_flops, 20e12);
  assert.equal(chip.sfu_ops, 5e12);
});
