...
// - 分派：FROM_NODE[operator_id] 查表（flop_registry）；挂到 FORMULAS[id].fromNode。
...
function linearStateUpdateMacs(config, { batch = 1, sequence = 1, phase = "prefill" } = {}) {
  ...
  const stateUpdate = recipeLinearAttentionMode(config) === "generic"
...
function stateUpdateCounts(config, options, bytesPerElement, modelKind = "") {
  ...
  return {
    ...
    // 不跟激活字节宽 —— 与 paramDtypes 登记表锁步。
    bytes: { weights: gdnScalars * paramBytes("gdn_decay"), actIn: stateBytes * steps, actOut: stateBytes * steps },
  };
}
// ---------- 旧链镜像结束 ----------
const FROM_NODE = {
  linear: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  matmul: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  sdpa_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  qsa_sparse_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  minimax_sparse_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  dsv4_swa_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  dsv4_compressed_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  softmax: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  rope: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  rmsnorm: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  gated_rmsnorm: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return rmsnormCounts({
            tokens, hidden: staticWidth(node?.input_shape) || 0, bytesPerElement, gated: true,
            weightWidth: normWeightWidth(node?.input_shape) || undefined,
          });
  },
  attention_output_gate: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return gateCounts({ tokens, width: staticWidth(node?.output_shape) || 0, bytesPerElement });
  },
  vision_activation: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return swigluCounts({ tokens, intermediate: staticWidth(node?.output_shape) || 0, bytesPerElement });
  },
  vision_position: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return addCounts({ tokens, hidden: staticWidth(node?.output_shape) || 0, bytesPerElement });
  },
  vision_merge: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  split: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return rearrangeCounts();
  },
  swiglu: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return swigluCounts({ tokens, intermediate: staticWidth(node?.output_shape) || 0, bytesPerElement });
  },
  fused_moe_mlp: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {

          // N2-4 W-A：MoE 路由专家融合叶（gate/up/down GEMM + SwiGLU 激活），分派
          // 只看 operator_id（W3 的「换 attributes 标记」到此落地）。
  ...
  causal_conv1d: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  linear_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  gated_delta_attention: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return stateUpdateCounts(config, { batch: options.batch ?? 1, sequence: options.sequence ?? 1, phase }, bytesPerElement, String(node?.attributes?.model_kind || ""));
  },
  topk: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  moe_dispatch: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  moe_combine: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  residual_add: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return addCounts({ tokens, hidden: staticWidth(node?.output_shape) || config?.hiddenSize || 0, bytesPerElement });
  },
  identity: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return rearrangeCounts({ copy: false });
  },
  moe_add: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    return addCounts({ tokens, hidden: staticWidth(node?.output_shape) || config?.hiddenSize || 0, bytesPerElement });
  },
  dsv4_hash_route: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
  ...
  mla_query_compress: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    ...
    return { ...FORMULAS.mla_query_compress.counts(ctx) };
  },
  mla_kv_compress: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.mla_kv_compress.counts(ctx) };
  },
  qsa_indexer: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.qsa_indexer.counts(ctx) };
  },
  dsa_indexer: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.dsa_indexer.counts(ctx) };
  },
  dsa_kpool_indexer: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
          heads: config?.dsaIndexHeads ?? config?.indexerNHeads ?? 0,
          dim: config?.dsaIndexHeadDim ?? config?.indexerHeadDim ?? 0,
    ...
    return { ...FORMULAS.dsa_kpool_indexer.counts(ctx) };
  },
  dsv4_indexer: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
          heads: config?.dsaIndexHeads ?? config?.indexerNHeads ?? 0,
          dim: config?.dsaIndexHeadDim ?? config?.indexerHeadDim ?? 0,
    ...
    return { ...FORMULAS.dsv4_indexer.counts(ctx) };
  },
  minimax_sparse_indexer: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.minimax_sparse_indexer.counts(ctx) };
  },
  attention_residual: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.attention_residual.counts(ctx) };
  },
  hyper_connection: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = (() => {
    ...
    return { ...FORMULAS.hyper_connection.counts(ctx) };
  },
  ple: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.ple.counts(ctx) };
  },
  mhc_pre: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.mhc_pre.counts(ctx) };
  },
  mhc_post: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.mhc_post.counts(ctx) };
  },
  mhc_fused_post_pre: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
    ...
    return { ...FORMULAS.mhc_fused_post_pre.counts(ctx) };
  },
  mhc_contract: ({ node, config, options, path, bytesPerElement, operatorId, kind, vision, phase, tokens }) => {
    const H = config?.hiddenSize || 0;
    const ctx = {
          contract: { tokens, hidden: H, bytesPerElement },
        };
    return { ...FORMULAS.mhc_contract.counts(ctx) };
  },
};
...
/**
 * 计算单个算子节点的动作向量。分派 = FROM_NODE[operator_id]（flop_registry）。
...
export function countsForNode(node, env = {}) {
  ...
  const fromNode = FROM_NODE[effectiveOperatorId];
  if (!fromNode) {
    return effectiveOperatorId
      ? null
      : { matrix: 0, vector: 0, sfu: 0, bytes: { weights: 0, actIn: 0, actOut: 0 } };
  }
  return fromNode({ node, config, options, path, bytesPerElement, operatorId: effectiveOperatorId, kind, vision, phase, tokens });
}
