...
export default function CostSummary({ structure, chips = PUBLIC_CHIPS, onAddChip, language = "zh", onFitStatusChange, lenses: controlledLenses, onLensesChange, phase: controlledPhase, onPhaseChange, mode: controlledMode, onModeChange, plans: controlledPlans, onPlansChange, nodes: controlledNodes, onNodesChange, gpusPerNode: controlledGpusPerNode, onGpusPerNodeChange, machineId: controlledMachineId, onMachineIdChange, loads: controlledLoads, onLoadsChange, comparisonMode = "off", onComparisonModeChange, compareChipId = "", onCompareChipIdChange, comparePlan = DEFAULT_COMPARE_PLAN, onComparePlanChange, efficiency = DEFAULT_EFFICIENCY, onEfficiencyChange }) {
  ...
  const peakCost = useMemo(() => {
    ...
    return aggregateCost({ graph: structure.graph, config, parameterCount: structure.summary?.parameters_by_dtype, phase, batch: load.batch, sequence: Math.min(load.sequence, load.chunkSize), kvBytes: kvElementBytes, activationPeak: activationGiB * GIB, runtimeConst: runtimeGiB * GIB, commBuffer: commBufferGiB * GIB, visionTokens: load.visionTokens ?? 1024, weightBytesPerParameter: weightMode === "actual" ? undefined : Number(weightMode) });
  ...
  const planFitsMemory = mode === "pd"
    ? pdFit?.[phase]?.fit
    : projected?.ok
      ? projected.stages.every((stage) => stage.weightBytes + stage.kvBytes + (stage.stateBytes || 0) + peakCost.memory.activationBytes + cost.memory.runtimeBytes + cost.memory.commBufferBytes <= available)
  ...
  return <section className="cost-summary cost-summary-modern" aria-label={text.estimate}>
    ...
    {expanded && <div className="cost-config-modern">
      ...
      <div className="cost-config-section"><h4>Cost assumptions</h4><div className="cost-config-grid"><label>KV bytes / element<select value={kvElementBytes} onChange={(event) => setKvElementBytes(Number(event.target.value))}><option value="2">2</option><option value="1">1</option><option value="0.5">0.5</option></select></label><label>Activation peak / GiB<input type="number" min="0" step="0.1" value={activationGiB} onChange={(event) => setActivationGiB(Math.max(0, Number(event.target.value) || 0))} /></label><label>Runtime / GiB<input type="number" min="0" step="0.1" value={runtimeGiB} onChange={(event) => setRuntimeGiB(Math.max(0, Number(event.target.value) || 0))} /></label><label>Comm buffer / GiB<input type="number" min="0" step="0.1" value={commBufferGiB} onChange={(event) => setCommBufferGiB(Math.max(0, Number(event.target.value) || 0))} /></label><label className="cost-check"><input type="checkbox" checked={interNode} onChange={(event) => setInterNode(event.target.checked)} /> {english ? "Inter-node link (roofline comm)" : "跨节点链路（roofline 通信）"}</label><label>Weight what-if<select value={weightMode} onChange={(event) => setWeightMode(event.target.value)}><option value="actual">actual / derived</option><option value="2">BF16 / FP16</option><option value="1">FP8 / INT8</option><option value="0.5">INT4</option></select></label></div></div>
    </div>}
    <div className="cost-breakdown">{[["Weights", cost.memory.weightBytes], ["Buffers", cost.memory.bufferBytes || 0], ["KV", cost.memory.kvBytes], ["KDA state", cost.memory.stateBytes], ["Activation peak", peakCost.memory.activationBytes], ["Runtime", cost.memory.runtimeBytes], ["Communication buffer", cost.memory.commBufferBytes]].map(([label, value]) => <span key={label}><b>{label}</b>{formatBytes(value)}</span>)}</div>
    ...
    <div className="cost-assumptions">{phase === "prefill" && load.chunked ? `${text.chunkedSummary(load.sequence, Math.min(load.sequence, load.chunkSize))} ` : ""}{cost.weightSource === "derived-quantized" ? `Weights use a ${cost.assumptions.weightBytesPerParameter} B/parameter ${cost.assumptions.quantization} estimate; checkpoint metadata can refine module exceptions. ` : ""}{text.theoretical} {roofline && `Roofline ${summary.times.map((t) => `${t.label} ${t.known ? formatSeconds(t.seconds) : (english ? "n/a" : "未知")}`).join(" · ")}.`}{roofline && roofline.bound === "unknown" && summary.missingCount > 0 && ` ${english ? `Roofline unknown because missing: ${summary.missingLabels.join(", ")}.` : `Roofline 未知——缺失：${summary.missingLabels.join("、")}。`}`}{summary.unknownComputeCount > 0 && ` ${english ? `${summary.unknownComputeCount} operators have no cost formula.` : `${summary.unknownComputeCount} 个算子无成本公式。`}`}}</div>
  </section>;
}
