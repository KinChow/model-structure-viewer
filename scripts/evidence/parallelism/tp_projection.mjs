// parallelism/tp_projection.mjs —— 前端并行策略（TP）投影 dump：projectPlan/kvBytesPerCard/ringAllReduceBytes。
// 供与真实 SGLang TP 每卡 weight/KV 对账。复用生产 API（cost/parallel.js、cost/comm.js）。回填 evidence/parallelism/tp_parallel.md。
// 用法：node scripts/evidence/parallelism/tp_projection.mjs [<config.json>]（默认取 MSV_PROBE_MODEL/config.json）。
...
const cfgPath = process.argv[2]
  || (process.env.MSV_PROBE_MODEL ? path.join(process.env.MSV_PROBE_MODEL, "config.json") : "<path-to>/Qwen3-0.6B/config.json");
...
const dir = process.env.MSV_EVIDENCE_OUT || path.join(repoRoot, "_evidence_out/parallelism");
