...
2026-09-09 定稿。状态：**W-A 已落地**（fused_moe_mlp 独立 id + 层 1 声明 +
锚 1 全目录 5233 叶容差 0；W-B 消费者接线、W-C 覆盖推进待做）。上文问题链：
量化容量逐矩阵化（N2-3）→ 显形 swiglu 携带专家 GEMM 未枚举（N2-4 登记）→
EP 维度补刀（用户）→ 联网调研定稿（本文件，`188679b`/`c3ed249` 两轮迭代）。
