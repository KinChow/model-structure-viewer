...
export default function CostSummary({ structure, chips = PUBLIC_CHIPS, onAddChip, language = "zh", lenses: controlledLenses, onLensesChange, phase: controlledPhase, onPhaseChange, mode: controlledMode, onModeChange, plans: controlledPlans, onPlansChange, nodes: controlledNodes, onNodesChange, gpusPerNode: controlledGpusPerNode, onGpusPerNodeChange }) {
  ...
  return <section className="cost-summary cost-summary-modern" aria-label={text.estimate}>
    ...
    {mode === "pd" && <div className="pd-deployment-summary"><span><b>Prefill</b> · {nodes.prefill} {nodes.prefill === 1 ? "node" : "nodes"} × {gpusPerNode} GPU · TP{plans.prefill.tp} / PP{plans.prefill.pp} / EP{plans.prefill.ep} / DP{plans.prefill.dp} · fit {fitText(pdFit?.prefill?.fit)}</span><span><b>Decode</b> · {nodes.decode} {nodes.decode === 1 ? "node" : "nodes"} × {gpusPerNode} GPU · TP{plans.decode.tp} / PP{plans.decode.pp} / EP{plans.decode.ep} / DP{plans.decode.dp} · fit {fitText(pdFit?.decode?.fit)}</span></div>}
    {expanded && <div className="cost-config-modern">
      <div className="cost-config-section"><h4>Machine node</h4><div className="cost-config-grid"><label>GPU / 芯片<select value={machine.id} onChange={(event) => setMachineId(event.target.value)}>{chips.map((chip) => <option key={chip.id} value={chip.id}>{chip.name}</option>)}</select></label>{mode === "centralized" && <label>Nodes<input type="number" min="1" value={nodes.centralized} onChange={(event) => updateNodes({ ...nodes, centralized: Math.max(1, Number(event.target.value) || 1) })} /></label>}<label>GPU / Node<input type="number" min="1" value={gpusPerNode} onChange={(event) => updateGpusPerNode(Math.max(1, Number(event.target.value) || 1))} /></label><div className="cost-machine-spec">{formatBytes(machine.memory_bytes)} / card · {formatBytes(machine.memory_bandwidth)} HBM · {Object.entries(machine.peak_flops || {}).map(([dtype, value]) => `${dtype.toUpperCase()} ${formatMacs(value)}`).join(" · ") || "算力未知"}</div></div><ManualChipForm onAdd={(chip) => { onAddChip?.(chip); setMachineId(chip.id); }} /></div>
