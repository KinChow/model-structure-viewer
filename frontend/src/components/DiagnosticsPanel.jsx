...
// 诊断码 → i18n catalog 键（{code, params} 渲染；缺键回退英文 message）。
const DIAG_KEY = {
  "missing-layer-count": "diag.missingLayerCount",
  "unsupported-architecture": "diag.unsupportedArchitecture",
};

function diagText(entry, language) {
  const key = entry?.code && DIAG_KEY[entry.code];
  return key ? t(language, key, entry.params) : entry?.message;
}
